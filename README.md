...
### 📦 6. 缓存布局
- 对于首次打开的网络图，会自动在成功打开后将计算完毕的结点位置缓存，之后打开时便可直接读取缓存文件，避免多次计算浪费时间。
  - 再次打开网络图时，边的形状会被重绘为直线，进而导致和初次打开时有视觉上的差异。这是，很可惜，无法克服的问题。
...
- 功能概览：
  ...
  - 输入 `g` 以主动查找根目录下所有 `.map` 文件
    ...
    - 生成时，会自动清除缓存并调用 `visualize_triggers.py`
...
的方法同样可用，不过这种方式不会清除旧缓存/更新基于前述注意事项，更推荐使用客户端。
